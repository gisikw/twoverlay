...
function messageWithEmoji(message, emotes, pct) {
  ...
  const replacements = Object.keys(emotes).reduce((h, icon) => (
    emotes[icon].reduce((o, markers) => Object.assign({}, o, { [markers]: icon }), {})
  ), {});
  ...
  Object
    ...
    .forEach((marker, i) => {
      ...
      msgArray.push(<img key={i} alt="emoji" style={styles.emoji(pct)} src={`http://static-cdn.jtvnw.net/emoticons/v1/${replacements[marker]}/1.0`} />);
