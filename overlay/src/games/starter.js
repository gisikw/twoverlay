...
function game({ id, width, height, subscribe }) {
  const Phaser = global.Phaser;
  let targetX;
  let targetY;
  let player;
  const game = new Phaser.Game(width, height, Phaser.AUTO, id,
                               { preload, create, render }, true);

  pickRandomTarget();
  subscribe(pickXYTarget);
  ...
  function pickRandomTarget() {
    targetX = Math.random() * (width - (AVATAR_SCALE * SPRITE_SIZE));
    targetY = Math.random() * (height - (AVATAR_SCALE * SPRITE_SIZE));
  }

  function pickXYTarget(x, y) {
    targetX =
      Math.max(Math.min(x, width - (AVATAR_SCALE * SPRITE_SIZE)), 0);
    targetY =
      Math.max(Math.min(y, height - (AVATAR_SCALE * SPRITE_SIZE)), 0);
  }
}
